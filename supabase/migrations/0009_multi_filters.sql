-- ---------------------------------------------------------------------------
-- Multi-select filters, has-photo filter, and SQL-side facet counting
--
-- Reworks search + faceting so the sidebar can:
--   * select MULTIPLE values per group (OR within a group, AND across groups)
--   * filter to listings that have uploaded photos
--   * compute facet counts in the database (no 1000-row JS cap)
--
-- search_listings now takes array params (p_types, p_conditions, …) plus
-- p_has_photo, and listing_facets returns all counts as one jsonb object using
-- standard "exclude-self" semantics (each group counts with every OTHER active
-- filter applied but its own excluded).
--
-- NOTE: search_listings changes signature, so the old single-value version is
-- dropped first. Apply this migration before the matching app code, or search
-- breaks. Idempotent — safe to re-run.
-- ---------------------------------------------------------------------------

-- Old single-value signature (from 0001) — drop so the new array version can
-- take its place instead of creating an ambiguous overload.
drop function if exists public.search_listings(
  text, listing_type, text, numeric, numeric, book_condition, text, integer, text, integer, integer
);
...
-- Facet counts for the filter sidebar, computed in one round-trip. Returns:
--   { total, types{}, conditions{}, genres{}, cities{}, languages{} }
-- Each group is counted with all OTHER active filters applied but its own
-- excluded (so a group still shows the alternatives you could switch to).
-- has_photo and price are plain filters (no group of their own), so they apply
-- to every count.
