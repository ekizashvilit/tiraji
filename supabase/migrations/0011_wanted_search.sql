-- ---------------------------------------------------------------------------
-- Wanted listings: keep them out of normal search/facets
--
-- Redefines search_listings and listing_facets so 'wanted' posts are excluded
-- by default and only returned when explicitly requested (the /wanted page
-- passes p_types => {wanted}). Everything else is unchanged from 0009.
--
-- Requires 0010 (the 'wanted' enum value) to be COMMITTED first.
