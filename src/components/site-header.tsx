  ...
  const navLinks = SECTIONS.map((s) => {
    ...
    return (
      <Link
        ...
        className={cn(
          "caps relative px-3 py-3.5 text-[0.9rem] font-medium transition-colors hover:text-primary",
    ...
    <header className="sticky top-0 z-40 bg-background">
      ...
      <div className="border-b border-border">
        <div className="mx-auto flex h-16 max-w-6xl items-center gap-4 px-4">
          ...
          >
            ...
            <span className="caps text-2xl font-bold text-brand-dark">
              {"ტირაჟი".toUpperCase()}
            </span>
          </Link>
      ...
      <div className="border-b border-border">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-3">
          ...
          <Link
            href="/sell"
            className="caps flex items-center gap-1.5 px-3 py-3.5 text-[0.9rem] font-semibold text-primary hover:underline"
          >
          ...
          <nav className="mx-auto flex max-w-6xl flex-col gap-1 px-4 py-3">
            {SECTIONS.map((s) => (
              <Link
                ...
                href={s.href}
                onClick={() => setOpen(false)}
                className="rounded-md px-3 py-2 text-[0.95rem] font-medium hover:bg-accent"
              >
                {t(s.key)}
              </Link>
            ))}
            <Link
              href="/sell"
              onClick={() => setOpen(false)}
              className="rounded-md px-3 py-2 text-[0.95rem] font-semibold text-primary hover:bg-accent"
            >
              {t("sell")}
            </Link>
