	...
	return (
		<>
			<PageHeader
				...
				crumbs={[{ label: tNav("home"), href: "/" }, { label: t("searchResultsTitle") }]}
			/>
			<div className="mx-auto max-w-6xl px-4 py-8">
				<div className="lg:grid lg:grid-cols-[220px_1fr] lg:gap-8">
					<aside className="lg:pr-2">
						<SearchFilters genres={genres} facets={facets} locale={locale} resultCount={facets.total} />
					</aside>
					...
					<div className="min-w-0">
						<div className="mb-5 hidden flex-wrap items-center justify-between gap-3 border-b border-border pb-4 lg:flex">
							<p className="text-sm text-muted-foreground">
								{sp.q ? (
									<>
										<span className="font-semibold text-foreground">{tf("results", { count: facets.total })}</span> · {t("searchingFor", { query: sp.q })}
									</>
								) : (
									<span className="font-semibold text-foreground">{tf("results", { count: facets.total })}</span>
								)}
							</p>
							<SortSelect />
						</div>
						...
						{listings.length > 0 ? (
							...
							<div className="flex flex-col items-center gap-6 rounded-xl bg-card px-6 py-16 text-center">
								...
								<div>
									<p className="max-w-sm text-muted-foreground">{sp.q ? t("noResultsFor", { query: sp.q }) : t("resultsSoon")}</p>
								</div>
								{/* Send them to the alerts page, carrying the search term to prefill. */}
								<Button asChild>
									<Link href={sp.q ? `/account/alerts?title=${encodeURIComponent(sp.q)}` : "/account/alerts"}>{tAlerts("searchCta")}</Link>
								</Button>
							</div>
						)}
					</div>
