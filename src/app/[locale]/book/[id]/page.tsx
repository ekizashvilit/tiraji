...
> = {
  sale: {
    ...
    navKey: "buy",
    typeKey: "typeSale",
    pill: "bg-buy/10 text-buy",
  },
  swap: {
    href: "/swap",
    navKey: "swap",
    typeKey: "typeSwap",
    pill: "bg-swap/10 text-swap",
  },
  giveaway: {
    href: "/giveaway",
    navKey: "giveaway",
...
export default async function BookPage({ params }: Params) {
  ...
  const memberSince = seller
    ? monthFmt.format(new Date(seller.created_at))
    : null;
  ...
  if (conditionLabel)
    details.push({ label: tf("condition"), value: conditionLabel });
  if (listing.genre)
    details.push({
      label: tf("genre"),
      value: genreName(listing.genre, locale),
    });
  ...
  if (listing.city)
    details.push({ label: tf("city"), value: cityLabel(listing.city, locale) });
