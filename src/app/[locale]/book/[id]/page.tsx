...
const SECTION: Record<
  ListingType,
  { href: string; navKey: string; typeKey: string; pill: string }
> = {
...
export default async function BookPage({ params }: Params) {
  ...
  const conditionLabel = listing.condition
    ? tf(CONDITION_KEY[listing.condition])
    : null;
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      <Breadcrumbs
        items={[
          ...
          { label: tNav(section.navKey), href: section.href },
          { label: listing.title },
        ]}
      />
      ...
      <div className="mt-5 grid gap-8 lg:grid-cols-[minmax(0,200px)_minmax(0,1fr)_minmax(0,300px)]">
        ...
        <div className="min-w-0 space-y-4">
          <div>
            <h1 className="text-2xl font-bold leading-tight sm:text-[1.7rem]">
              {listing.title}
            </h1>
            {listing.author && (
          ...
          <div className="space-y-1 text-sm text-muted-foreground">
            {listing.isbn && (
              <p>
                {t("isbn")}:{" "}
                <span className="text-foreground">{listing.isbn}</span>
              </p>
            )}
            {listing.book_language && (
              <p>
                {tf("language")}:{" "}
                <span className="text-foreground">
                  {languageLabel(listing.book_language, locale)}
                </span>
              </p>
            )}
            <p>
              {t("posted")}: <span className="text-foreground">{posted}</span>
            </p>
          </div>

          {/* Condition / genre chips */}
          {(conditionLabel || listing.genre) && (
            <div className="flex flex-wrap gap-2">
              {conditionLabel && (
                <span className="rounded border border-border px-2 py-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  {tf("condition")}: {conditionLabel}
                </span>
              )}
              {listing.genre && (
                <span className="rounded border border-border px-2 py-1 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                  {genreName(listing.genre, locale)}
                </span>
              )}
            </div>
          )}
          ...
          <div className="space-y-0.5 pt-1 text-sm">
            <p className="text-muted-foreground">
              ...
              {listing.city ? `, ${cityLabel(listing.city, locale)}` : ""}
            </p>
            {memberSince && (
              <p className="text-muted-foreground">
      ...
      {similar.length > 0 && (
        <div className="mt-12">
          <BookShelf
            title={t("similar")}
            href={listing.genre ? `/search?genre=${listing.genre.slug}` : "/buy"}
            listings={similar}
          />
        </div>
      )}
